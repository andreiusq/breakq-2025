...
class QuantumFeatureExtractor(nn.Module):
    ...
    def __init__(self, n_qubits=4, n_layers=3, encoding='angle', ansatz='basic'):
        ...
        self.qlayer = qml.qnn.TorchLayer(self._qnode, weight_shapes)

    def _qnode(self, inputs, ansatz_weights):
        ...
        def circuit(inputs, ansatz_weights):
            for layer in range(self.n_layers):
                if self.encoding == 'angle':
                    ...
                    norm_inputs = inputs[:self.n_qubits]
                    norm = torch.sqrt(torch.sum(norm_inputs**2))
                    qml.AmplitudeEmbedding(norm_inputs / (norm + 1e-8),
                ...
                elif self.ansatz == 'strong':
                    for q in range(self.n_qubits):
                        qml.CNOT(wires=[q, (q+1) % self.n_qubits])

            return [qml.expval(qml.PauliZ(i)) for i in range(self.n_qubits)]

        return circuit(inputs, ansatz_weights)
...
class EntanglementTracker:
    ...
    def compute_entanglement(state_vector, qubit_partition):
        ...
        return len(qubit_partition) * 0.5  # TO_DO: Density Matrix [Placeholder!!!]
...
class QuantumFirstHybrid(nn.Module):
    ...
    def __init__(self, num_classes, in_channels=1, n_qubits=4, n_layers=3):
        ...
        self.fusion = nn.Sequential(
            nn.Flatten(),
            nn.Linear(4 * 4 * n_qubits, 32),  # 4x4 spatial, n_qubits features each
            nn.ReLU(),
            nn.Dropout(0.2),
            nn.Linear(32, num_classes)
        )
    ...
    def forward(self, x):
        # Extract patches
        x = self.patch_extractor(x)  # (B, 8, 4, 4)

        # Process each spatial location through quantum circuit
        B, C, H, W = x.shape
        x = x.permute(0, 2, 3, 1).reshape(B * H * W, C)  # (B*H*W, 8)

        # Quantum processing (take first n_qubits features)
        x_quantum = self.quantum_feature(x[:, :self.n_qubits])  # (B*H*W, n_qubits)

        # Reshape back
        x_quantum = x_quantum.reshape(B, H, W, self.n_qubits).permute(0, 3, 1, 2)  # (B, n_qubits, H, W)
...
class QuantumLastHybrid(nn.Module):
    """Original approach: Classical CNN → Quantum post-processing"""
...
class ExperimentOrchestrator:
    """
    Run comprehensive ablation studies comparing all architectures
    ...
    def run_data_regime_study(self, dh_base, num_classes, in_channels,
        ...
        model_builders = {
            ...
            'Quantum-First': lambda: QuantumFirstHybrid(num_classes, in_channels, n_qubits=4, n_layers=2),
            'Quantum-Last': lambda: QuantumLastHybrid(num_classes, in_channels),
    ...
    def plot_data_regime_results(self, results, save_path='data_regime_study.png'):
        """Visualize low-data regime results"""
        ...
        # Plot 3: Parameter Efficiency (Accuracy per Parameter)
        ax = axes[1, 0]
        model_names = list(results.keys())
        # Use 100% data results
        full_data_idx = -1
        accs = [results[m]['test_acc'][full_data_idx] for m in model_names]
        params = [results[m]['n_params'][full_data_idx] for m in model_names]
        efficiency = [a / (p/1000) for a, p in zip(accs, params)]  # Acc per 1K params
...
def main():
    """
    Run the comprehensive quantum ML experiment
    """
    print("\n" + "="*80)
    ...
    # For faster experiments, you can select fewer classes
    # dh.select_classes([0, 1, 2])  # Uncomment for binary/3-class
    ...
    # Run low-data regime study (THE BRAVE EXPERIMENT!)
    results = orchestrator.run_data_regime_study(
        dh_base=dh,
        num_classes=num_classes,
        in_channels=in_channels,
        data_percentages=[0.05, 0.1, 0.25, 0.5, 1.0],  # 5%, 10%, 25%, 50%, 100%
    ...
    for model_name in results.keys():
        ...
        # Calculate improvement in low-data regime
